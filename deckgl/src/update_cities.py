...
with open('ne_count16.json') as json_file:
    ...
    for elem in data["features"]:
         ...
         if elem["properties"]["name"] in list(all_data.keys()):
              ...
              bb["properties"]["scalerank"]=sum(all_data[elem["properties"]["name"]]['3/7/20'])
              bb["properties"]["one_day"]=sum(all_data[elem["properties"]["name"]]['3/7/20'])-sum(all_data[elem["properties"]["name"]]['3/6/20'])
         else:
